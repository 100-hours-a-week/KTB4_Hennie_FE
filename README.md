...
확인이 필요한 동작은 공통 `ConfirmModal`로 처리합니다. 배경 스크롤 잠금, 포커스 이동과 트랩, `Escape` 닫기, 처리 중 중복 클릭 차단을 한곳에서 담당하고, 신고처럼 추가 입력이 필요한 경우 `children`으로 내용을 주입합니다.

| 모달           | 사용 위치                                   | 스크린샷                                                                                         | 확인할 기능                                    |
| -------------- | ------------------------------------------- | ------------------------------------------------------------------------------------------------ | ---------------------------------------------- |
| 삭제 확인      | 게시글·댓글·답글 삭제, 수정 취소, 회원 탈퇴 | <img src="./docs/screenshots/modal-confirm.png" width="560" alt="개발바닥 삭제 확인 모달" />     | 되돌릴 수 없는 동작 안내, 취소·확인 동등 배치  |
| 신고           | 게시글 상세의 신고 액션                     | <img src="./docs/screenshots/modal-report.png" width="560" alt="개발바닥 게시글 신고 모달" />    | 사유 단일 선택, 선택 상태 강조, 미선택 시 차단 |
| 임시 저장 목록 | 게시글 작성 화면의 임시 저장 카운터         | <img src="./docs/screenshots/modal-drafts.png" width="560" alt="개발바닥 임시 저장 목록 모달" /> | 임시 저장 불러오기·삭제, 편집 중 항목 구분     |
